...
<head>
    <meta charset="UTF-8">
    <title>Cetak Laporan Rapat yang Sering Dibahas</title>
    <style>
        body {
...
<body onload="window.print()">
    ...
    <h3>Laporan Rapat yang Sering Dibahas</h3>
    <?php if (!empty($bulan)): ?>
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): $no = 1; ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= $no++ ?></td>
                        <td style="text-align:left;"><?= htmlspecialchars($row['judul_rapat']) ?></td>
                        <td><?= $row['jumlah_dibahas'] ?></td>
                        <td><?= date('d-m-Y', strtotime($row['tanggal_pertama'])) ?></td>
                        <td><?= date('d-m-Y', strtotime($row['tanggal_terakhir'])) ?></td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr>
